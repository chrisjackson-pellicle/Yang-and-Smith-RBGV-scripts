...
def main(DIR, tree_file_ending, relative_cut, absolute_cut, output_folder):
    if DIR[-1] != "/": DIR += "/"
    filecount = 0
    ...
    for i in os.listdir(DIR):
        if i.endswith(tree_file_ending):
            ...
            if outtree != None:
                with open(output_folder + '/' + i + ".tt", "w") as outfile:
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("python 04_trim_tips.py DIR tree_file_ending relative_cutoff absolute_cutoff output_folder")
        sys.exit(0)

    DIR, tree_file_ending, relative_cut, absolute_cut, output_folder = sys.argv[1:]
    main(DIR, tree_file_ending, relative_cut, absolute_cut, output_folder)
