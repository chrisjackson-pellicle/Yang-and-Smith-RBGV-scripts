...
"""
- Adds an outgroup to each fasta file generated by HybPiper's paralog_retriever.
- Creates a 'in_and_outgroups_list.txt' file for use in the Y&S pruning scripts.
- Aligns the fasta file with outgroup added using mafft, and if the option -no_supercontigs is provided, realigns using
  Clustal Omega (which can do a better job when alignment contains contigs from different regions of the full-length
    ...
    for alignment in glob.glob(f'{input_folder}/*.aln.trimmed.fasta'):
        ...
        try:
            ...
            try:
                ...
                with open(hmm_file, 'w') as filtered_hmm_fasta:
...
def mafft_align_multiprocessing(fasta_to_align_folder, alignments_output_folder, algorithm='linsi', pool_threads=1,
    ...
    logger.info('Generating alignments for fasta files using mafft...\n')
...
def main():
    ...
    folder_02 = f'{cwd}/03_alignments_hmmcleaned'

    # outgroups_dict = get_outgroup_seqs(results.target_file, results.outgroups)
    # append_outgroup_seqs(results.gene_fasta_directory, outgroups_dict, folder_01)
    if not results.no_supercontigs:  # i.e. if it's a standard run with supercontigs produced.
        logger.info(f'Running without no_supercontigs option - aligning with mafft only')
        ...
        logger.info(f'Running with no_supercontigs option - realigning with clustal omega')
