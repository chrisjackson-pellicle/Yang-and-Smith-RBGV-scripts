...
def strip_names_for_concat(input_folder, output_folder):
    """
...
def main():
    ...
    folder_01 = f'{cwd}/16_alignments_stripped_names'
    folder_02a = f'{cwd}/17a_alignments_stripped_names_realigned_mafft'
    folder_02b = f'{cwd}/17_alignments_stripped_names_realigned'

    output_folder = strip_names_for_concat(results.gene_fasta_directory, folder_01)

    if not results.no_supercontigs:  # i.e. if it's a standard run.
        mafft_align_multiprocessing(output_folder, folder_02b, algorithm='linsi', pool_threads=results.threads_pool,
                                    mafft_threads=results.threads_mafft)
    elif results.no_supercontigs:  # re-align with Clustal Omega.
        mafft_align_multiprocessing(output_folder, folder_02a, algorithm='linsi', pool_threads=results.threads_pool,
                                    mafft_threads=results.threads_mafft)
        clustalo_align_multiprocessing(folder_02a, folder_02b, pool_threads=results.threads_pool,
