...
def main(treDIR, clnDIR, para, output_directory, intree_file_ending=INTREE_FILE_ENDING):
    if treDIR[-1] != "/": treDIR += "/"
    if clnDIR[-1] != "/": clnDIR += "/"
    ...
    for i in os.listdir(treDIR):
        if i.endswith(intree_file_ending):
            ...
            for s in read_fasta_file(clnDIR + filematch[clusterID]):
                s.name = s.name.split()[0]  # CJJ necessary to remove extraneous information that otherwise gets captured in the
            ...
            with open(output_directory + '/' + i + ".mm", "w") as outfile:
                outfile.write(newick3.tostring(curroot) + ";\n")
    assert filecount > 0, \
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("python mask_tips_by_taxonID_transcripts.py treDIR aln-clnDIR mask_paraphyletic(y/n) output_directory")
        sys.exit(0)

    treDIR, clnDIR, para, output_directory = sys.argv[1:]
    createfolder(output_directory)
    main(treDIR, clnDIR, para, output_directory)
